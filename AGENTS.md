...
### Cloud render (server-side FFmpeg)

`render-service/` is a self-contained Node service (Hono + @napi-rs/canvas + FFmpeg) that reproduces the client pipeline server-side — a vendored `canva-utils` copy gives pixel-parity overlays (verified: golden overlay diff 99.6% within tolerance, mean diff 0.735). Same timing constants (cumulative segments, 8-frame verse crossfades alpha-sum-1, 10-frame bg seams, 30fps doubled to 60), same encode ladder (libx264 High, bitrate by pixel count, AAC 128k mono 48k, +faststart). API: `POST /api/render` (job spec) → `GET /api/render/:id` (SSE, same `{msg,pct}` milestones) → `GET /api/render/:id/video`. Limits: 2 concurrent, queue depth 4, TTL 30min, per-IP token bucket, ~10-minute output cap. Deploy: **Render free tier** (no card — works at 512MB after the `x264-params ref=1:rc-lookahead=8:bframes=0` memory profile; ~4-6 min renders on 0.1 CPU, sleeps after 15 min idle) or **Google Cloud Run** (fastest — free tier ≈ thousands of renders/month, scale-to-zero; `scripts/deploy-cloud-run.sh` sets the required `--timeout 3600 --concurrency 1` flags) or any ≥2 vCPU/4GB Docker host — full guides in `render-service/DEPLOY.md`; `NEXT_PUBLIC_RENDER_API_URL` is the only env var. Client: `lib/render-client.ts` + `handleGenerate` in `VideoBuilder.tsx` try cloud first and fall back to the in-browser WebCodecs pipeline on ANY failure (asleep host, timeout, queue full, upload fail) — generation can never fully die. Browsers without WebCodecs skip the local fallback (cloud is their only path). **When `lib/canva-utils.ts` changes (text layout, fonts, overlays), re-copy it to `render-service/src/canvas/canva-utils.ts` and adapt imports to the local type shims** — the golden parity test (`render-service/test/`) is the drift alarm.
