...
**Renderer:** `lib/server/render-chunk.ts` (the old `render-service/` Docker port was deleted): same timing constants (cumulative segments, 8-frame verse crossfades alpha-sum-1, 10-frame bg seams), same encode ladder (libx264 High + `x264-params ref=1:rc-lookahead=8:bframes=0` memory profile, bitrate by pixel count, AAC 128k mono 48k). Every chunk uses IDENTICAL stream parameters → concat is lossless. Chunk-local bookend fades: fade-in only on chunk 0, fade-out only on the last chunk, or seams visibly pulse. **A/V SYNC (verified experimentally — don't regress):** the rawvideo stdin is declared `-r 30` and the output `-r 60`; each unique frame is written to the pipe ONCE — ffmpeg's frame duplication produces the 60fps track with exact timestamps. Writing each frame twice (the old double-write) feeds 2× frames to a 30fps input → video exactly 2× the audio duration (audio "cuts off" at the halfway point). **Bg pipeline (memory-bounded — don't regress):** frames are extracted as JPEG (mjpeg q4, ~10× smaller than PNG), kept encoded, and decoded on demand in the compositing loop through a 24-frame LRU cache; a 900-frame hard cap bounds stored memory. `extractBgFrames` REJECTS on non-zero exit (never resolves "0 frames"); each clip retries once and is DROPPED on failure; `hasBgVideo` is derived from ACTUAL extracted frames (not URL count) so failed bgs bake/paint the gradient fallback — never black. Upload-mode bytes probe via temp file. **When `lib/canva-utils.ts` changes, re-copy to `lib/server/server-canvas/canva-utils.ts`** (adapt imports to the local type shims). `next.config.js` keeps `@napi-rs/canvas`/`ffmpeg-static`/`ffprobe-static` in `serverExternalPackages` (webpack can't bundle native binaries — don't remove) AND `outputFileTracingIncludes` force-traces the runtime-loaded binaries into the serverless bundle: font TTFs + the LINUX-x64 ffmpeg/ffprobe binaries only (full `/**` globs pull every platform's binary → 450MB > Vercel's 250MB function limit — deploy fails). `lib/server/server-canvas/fonts.ts` resolves the TTF dir from `process.cwd()` first (the lambda extracts nft-traced files project-relative; `import.meta.url` in the bundle points at the BUILD-time path and ENOENTs) with a `__dirname` fallback for dev. The chunk route normalizes sparse spec settings (opacities, colors, `fontSize`, font families) BEFORE persisting the plan — a missing field would otherwise reach canvas as `rgba(0,0,0,NaN)` or `FONT_SIZES[undefined]`.

**Why chunked:** Hobby functions cap at 300s/1 vCPU/2GB. A ~55s-of-output chunk renders in ~120s at 1 vCPU, verified e2e on Windows dev (plan → 4 chunks → concat → valid 60fps H.264+AAC MP4, Arabic text pixel-verified) AND in production (plan → 2 chunks on Vercel Functions with real Blob/OIDC → valid MP4). Client chunk POSTs are sequential + idempotent + sessionStorage-tracked (`midhkar-render-job` — render-job state ONLY, not wizard settings), so refresh-resume and retries are safe. Blob auth is EITHER OIDC (`VERCEL_OIDC_TOKEN` + `BLOB_STORE_ID`, auto-injected when the store is connected to the project — the current setup) OR the legacy `BLOB_READ_WRITE_TOKEN`; `lib/server/render-store.ts` detects both LAZILY per call (a module-load check freezes build-time env into the bundle). **In Vercel Functions the OIDC token arrives via the `x-vercel-oidc-token` REQUEST HEADER, not process.env** — the SDK resolves it per request, so on Vercel a live `BLOB_STORE_ID` + a cheap `head()` probe (cached per instance) is the backend signal. **Client uploads MUST use the presigned flow** (`handleUploadPresigned` + `issueSignedToken` + `uploadPresigned`) — the legacy `handleUpload`/`upload` requires a static read-write token and FAILS with OIDC-only stores. Without Blob credentials, prod degrades to the browser pipeline and dev uses disk.

**Bg relay (Pexels datacenter-403 fallback — don't regress):** when every bg URL clip fails extraction server-side, `renderChunk` throws `BgUnavailableError` and the chunk route returns **422 `bg_unavailable`**. The CLIENT then fetches the same URLs (the browser's IP isn't blocked; Pexels sends `Access-Control-Allow-Origin: *`) and uploads them browser→Blob as `bg-relay-N` via the presigned flow, re-POSTing the chunk with `bgRelayed:true`. Loop-guards: the client relays at most ONCE per job; a failed relay retries the chunk with `noBg:true` (server skips the CDN, renders the gradient — never a second 422). With relayed bytes, any decode failure is NOT a fetch problem → gradient, no relay. `MAX_CHUNK_SEC = 30` (not 55): with real-bg compositing a chunk renders at ~7× realtime (measured in prod), so 30s × 7 + fetches ≈ 240s, safely under the 300s cap — full Fatihah (~46s) must therefore SPLIT into 2 chunks.

**Cancellation semantics (don't regress):** only a user click on Cancel shows "Cancelled." (`ctrl.signal.aborted` → AbortError). ANY other server-path failure (timeout, 5xx, upload fail, >10-min cap) falls through to the local WebCodecs pipeline with a log note — generation can never fully die.
...
### Product tour (`components/VideoBuilder/tour.tsx`)

"The Guided Studio" — 14 stops covering the whole wizard (4·Surah, 3·Verses, 4·Settings, 3·Generate), defined in the authored `STOPS` array. Anchors are `data-tour="…"` attributes on the real UI (`step-rail`, `surah-search`, `surah-popular`, `surah-index`, `verses-presets`, `verses-chip`, `verses-list`, `platform-cards`, `reciter-panel`, `customise-panel`, `side-preview`, `generate-preview`, `generate-summary`, `generate-button`) — pure attributes, zero logic in the step components. Phase A: welcome card (modal, once per browser, `midhkar-tour-done`). Phase B: coach-marks, non-blocking, auto-show the first unshown stop of a wizard step on arrival (`midhkar-tour-tips` mask — a stop shows at most once organically). **Next is step-local:** it walks only the current wizard step's stops, last one says Done; arrival at the next wizard step auto-shows its own first stop — never chain stops across steps (the user's screen doesn't have the anchor yet). The highlight ring + presence-poll retries attachment every 350ms (anchors mount a beat after the coach-mark during AnimatePresence step transitions) and self-dismisses only after a 2.6s grace + 4 consecutive misses (conditional anchors like `verses-chip` need a selection first). `isMobile` is captured ONCE at mount (bottom-sheet vs floating-tip-with-arrow) — resizing mid-tip doesn't re-layout until the next stop. "?" pill = full replay (session-only, never touches stored flags; Skip tour / final Done marks everything shown). All strings en/fr/ar inline; tip text ≥13px; reduced-motion safe (uses the global gate).
...
**Vercel Blob (render pipeline):** create a free Blob store in the Vercel dashboard (Storage → Create → Blob → Connect to Project). Connecting adds `BLOB_STORE_ID` + `VERCEL_OIDC_TOKEN` (+ `BLOB_WEBHOOK_PUBLIC_KEY`) — OIDC auth, rotated automatically. `vercel env pull` brings them to `.env.local` for dev (SDK auto-refreshes expired dev tokens via CLI auth). Legacy stores may expose `BLOB_READ_WRITE_TOKEN` instead — both work. Without Blob credentials, dev renders use a local disk store and production degrades to the in-browser WebCodecs pipeline.
